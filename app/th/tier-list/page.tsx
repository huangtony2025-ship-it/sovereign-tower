...
import { site, weapons, pageContent } from '@/lib/data.th';

export const metadata: Metadata = {
  title: pageContent.tierList.metaTitle ?? `อันดับอาวุธ ${site.name} — จัดอันดับอาวุธที่ดีที่สุด`,
  description:
    pageContent.tierList.metaDesc ?? `อันดับอาวุธ ${site.name}: จัดอันดับอาวุธทั้งหมดสำหรับการเอาชีวิตรอด ดูว่าอาวุธไหนนำเมต้าปัจจุบัน`,
  alternates: { languages: { 'x-default': `${site.url.replace(/\/th$/, '')}/tier-list` } },
};

export default function ThTierListPage() {
              ...
              {ordered
                ...
                .map((w) => (
                  <div className="card" key={w.name}>
                    <h3>{w.name}</h3>
                    <p><strong>{pageContent.tierList.bestWith ?? 'เข้ากับ'}:</strong> {w.synergy}</p>
                    <p><strong>{pageContent.tierList.playstyle ?? 'สไตล์เล่น'}:</strong> {w.playstyle}</p>
