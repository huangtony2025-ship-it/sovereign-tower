...
import { codes, pageContent } from '@/lib/data.th';

export default function ThCodesClient({ h1, desc }: { h1: string; desc: string }) {
  ...
  return (
    <main className="container" style={{ paddingTop: 48, paddingBottom: 48 }}>
      ...
      <section style={{ marginTop: 28 }}>
        <h2>{pageContent.codes.workingTitle ?? 'โค้ดที่ใช้งานได้'}</h2>
          ...
          {codes.working.map((c) => (
            <div className="code-item" key={c.code}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <code>{c.code}</code>
                {c.isNew && <span className="badge new" style={{ marginLeft: 8 }}>ใหม่</span>}
      ...
      {codes.expired.length > 0 && (
        <section style={{ marginTop: 32 }}>
          <h2>{pageContent.codes.expiredTitle ?? 'โค้ดที่หมดอายุ'}</h2>
          <p style={{ color: '#9a9ab0', marginTop: 8, fontSize: '0.9rem' }}>
            ...
            {codes.expired.map((c) => (
              <div className="code-item" key={c.code}>
                <div style={{ flex: 1 }}>
                  ...
                  <span className="badge expired" style={{ marginLeft: 8 }}>หมดอายุ</span>
                  <div className="reward">{c.reward}</div>
                </div>
              </div>
            ))}
      ...
      {codes.redeemSteps.length > 0 && (
        <section style={{ marginTop: 32 }}>
          <h2>{pageContent.codes.redeemTitle ?? 'วิธีแลกโค้ด'}</h2>
      ...
      <section style={{ marginTop: 32 }}>
        <h2>{pageContent.codes.moreTitle ?? 'วิธีหาโค้ดเพิ่ม'}</h2>
