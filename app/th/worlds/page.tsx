...
import { site, worlds, pageContent } from '@/lib/data.th';

export const metadata: Metadata = {
  title: pageContent.worlds.metaTitle ?? `คู่มือโลก ${site.name}`,
  description:
    pageContent.worlds.metaDesc ?? `คู่มือโลก ${site.name}: ผ่านโลกต่าง ๆ และเป็นนักสู้ที่เก่งที่สุด`,
  alternates: { languages: { 'x-default': `${site.url.replace(/\/th$/, '')}/worlds` } },
};

export default function ThWorldsPage() {
  return (
        ...
        <div className="grid">
          {worlds.map((w) => (
            <div className="card" key={w.name}>
              <span className="tag">{pageContent.worlds.cardTag ?? 'โลก'}</span>
              <h3>{w.name}</h3>
              <p>{w.note}</p>
            </div>
          ))}
